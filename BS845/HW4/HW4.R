...
1.	Generate a boxplot to compare the TOTALCHOL between hypertensive (HYPERTEN equal 1) and normal individuals. Make the box size proportional to sample size of each group. Compare the TOTALCHOL distributions between the two groups and comment on any similarity and difference between the two groups.
...
Please generate plant specific curves to display the relationship between  conc (response) and uptake (predictor) in four panels by Type and Treatment combinations using ggplot2 package. 
